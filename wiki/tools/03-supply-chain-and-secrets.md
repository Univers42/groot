...
| **pnpm overrides (selector + caret form)** | Force a vulnerable transitive up to a fixed version reliably | `apps/osionos/app/package.json:89` (`"qs@<6.15.2": "^6.15.2"`); `apps/opposite-osiris/package.json:75` (`"yaml@<2.8.3": "^2.9.0"`); `apps/opposite-osiris/package.json:72` (top-level npm-style override `devalue ^5.8.1`); `apps/osionos/app/package.json:82` (`pnpm.onlyBuiltDependencies` allowlist, 5 entries); `QUALITY-SECURITY-TOOLING.md:62` | `docker run --rm -v "$PWD:/repo" -w /repo/apps/<app> node:22-alpine sh -lc 'corepack enable && corepack prepare pnpm@11.5.1 --activate && pnpm install --lockfile-only'` | pnpm apps only (`osionos/app`, `opposite-osiris`) |
