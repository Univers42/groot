...
## 10 · SOC2 external auditor  💰

**Unblocks:** Compliance / *"SOC2 report"*. Gate **m108** (`m108-soc2-evidence.sh`) ships the
**SOC2-lite evidence collector** (hash-sealed ci/access/change_mgmt snapshots) — that is
*"evidence collected, audit-ready"*, **NOT "SOC2 certified"**. A certificate requires an
external auditor.

- 🔵 **a.** Engage a licensed SOC2 auditor (Vanta/Drata-assisted or a CPA firm). 💰
- ⚪ **b.** Hand them the evidence: enable `SOC2_EVIDENCE_ENABLED=1`, `POST /v1/compliance/collect`,
  export `GET /v1/compliance/evidence`. Trust-center posture is gate m112 (`config/trust/posture.json`).
