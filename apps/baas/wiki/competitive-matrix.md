...
| 6 | Native full-text search | v | ~ | **[x]** | No tenant FTS op exposed; stock-PG `tsvector` reachable only via admin-only `/v1/admin/raw` (`routes.rs:2178`), and `$like`/`$ilike` covers substring not ranked FTS. **Supabase honestly wins:** first-class `tsvector` FTS, managed | L | P2 | not in `operation.rs` (8-op surface); residual: first-class `op=fulltext_search` |
| 7 | Vector / embeddings | v | v | **[x]** | No pgvector / vector search; **pgvector is not even installed** (base image is stock `postgres:16-alpine`, `docker-compose.yml:483`) — no `<->`/`<=>` op anywhere. **Supabase honestly wins:** first-class `pgvector` (HNSW/IVFFlat), managed + documented | L | P2 | residual: ship `pgvector/pgvector` image + first-class `op=vector_search` |
