...
# **rest_insert**
> rest_insert(resource, rest_insert_request)
...
with grobase.ApiClient(configuration) as api_client:
    ...
    api_instance = grobase.RestApi(api_client)
    resource = 'resource_example' # str | Table or view name.
    rest_insert_request = grobase.RestInsertRequest() # RestInsertRequest | 

    try:
        # Insert one or many rows.
        api_instance.rest_insert(resource, rest_insert_request)
    except Exception as e:
...
------------- | ------------- | ------------- | -------------
 **resource** | **str**| Table or view name. | 
 **rest_insert_request** | [**RestInsertRequest**](RestInsertRequest.md)|  | 
...
# **rest_select**
> List[Dict[str, object]] rest_select(resource, select=select, order=order, limit=limit, offset=offset)
