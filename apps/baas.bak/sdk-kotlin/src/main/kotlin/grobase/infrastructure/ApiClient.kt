package grobase.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val BASE_URL_KEY: String = "grobase.baseUrl"
        @Deprecated(
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        if (requestConfig.headers["apikey"].isNullOrEmpty()) {
            if (apiKey["apikey"] != null) {
                if (apiKeyPrefix["apikey"] != null) {
                    requestConfig.headers["apikey"] = apiKeyPrefix["apikey"]!! + " " + apiKey["apikey"]!!
                } else {
                    requestConfig.headers["apikey"] = apiKey["apikey"]!!
                }
            }
        }
