//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:grobase/api.dart';
import 'package:test/test.dart';

// tests for TxnRequestOperationsInner
void main() {
  // final instance = TxnRequestOperationsInner();

  group('test TxnRequestOperationsInner', () {
    // String op
    test('to test the property `op`', () async {
      // TODO
    });

    // String resource
    test('to test the property `resource`', () async {
      // TODO
    });

    // Map<String, Object> data (default value: const {})
    test('to test the property `data`', () async {
      // TODO
    });

    // Map<String, Object> filter (default value: const {})
    test('to test the property `filter`', () async {
      // TODO
    });

    // String idempotencyKey
    test('to test the property `idempotencyKey`', () async {
      // TODO
    });


  });

}
