...
install_desktop_entry() {
  ...
  if [ -f "$icon_src" ]; then
    cp -f "$icon_src" "$icon_dir/track-binocle.png"
    icon_line="Icon=$icon_dir/track-binocle.png"
  fi
...
open_app() {
  if command -v xdg-open >/dev/null 2>&1; then
    xdg-open "$OSIONOS_URL" >/dev/null 2>&1 || true
...
launch | "")
  start_stack
  notify "Suite ready — opening osionos"
  open_app
  ;;
*)
  echo "usage: $0 [--install|launch|--stop]"
  exit 1
  ;;
esac
